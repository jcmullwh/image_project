...
def _read_generations_csv_v2(path: Path) -> tuple[dict[str, dict[str, Any]], list[str]]:
    """
    Read a v2 generations CSV and return a mapping keyed by generation_id.

    Required columns (case-insensitive; punctuation normalized):
    - generation_id
    - selected_concepts
    - final_image_prompt
    - image_path
    ...
    Legacy schema sniffing/parsing has been removed. If you have historical `generations.csv`
    files with headers like "ID" / "Description Prompt" / "Generation Prompt" / "Image URL",
    migrate them explicitly using `tools/migrate_generations_csv_legacy_to_v2.py`.
    """
    ...
    try:
        with open(path, "r", encoding="utf-8-sig", newline="") as handle:
            ...
            if missing:
                return {}, [
                    f"{path}: unsupported generations CSV schema; missing columns={missing} header={header}. "
                    "Expected v2 columns: "
                    f"{list(required)}. "
                    "If this is a legacy generations.csv, migrate it with: "
                    "python tools/migrate_generations_csv_legacy_to_v2.py --input <path> --output <path>"
                ]

            generation_id_key = header_map["generation_id"]
            selected_key = header_map["selected_concepts"]
            prompt_key = header_map["final_image_prompt"]
            image_key = header_map["image_path"]
            created_key = header_map["created_at"]
            ...
            for idx, row in enumerate(reader, start=2):
                ...
                selected = str(value).strip() or None if value is not None else None

                value = row.get(prompt_key)
                final_prompt = str(value).strip() or None if value is not None else None

                value = row.get(image_key)
                image_path = str(value).strip() or None if value is not None else None

                value = row.get(created_key)
                created_at = str(value).strip() or None if value is not None else None

                seed = _safe_int(row.get(seed_key))
...
def update_artifacts_index(
    *,
    ...
    for path in generation_csv_paths:
        records, errors = _read_generations_csv_v2(path)
